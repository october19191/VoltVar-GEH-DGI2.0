...
/*
  BOOST_FOREACH(CPeerNode peer, m_peers | boost::adaptors::map_values)//for broadcast use
	...
	*/
	...
	CPeerNode peer = CPeerNode("baffin.ece.ncsu.edu:5002");  //create an object of CpeerNode and set the UUID / hostname to the peer you want
	mat S1;
	S1 << Dl(0,7) << Dl(1,7) << Dl(2,7) << endr;
	S1 = S1.t();//has to be in column!
	ModuleMessage mg1 = Gradient(S1);
	peer.Send(mg1);
	//cout << "Vector sent: \n" << S1 << endl;
}
else
